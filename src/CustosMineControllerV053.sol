...
/**
 * @title CustosMineController v0.5.3
 ...
 *
 * Changes from v0.5.2:
 *   - Epoch-relative round numbering: each round has an epochRoundNumber (1-140)
 *   - E69 check now per-epoch (was global — capped total rounds at 140 across all epochs)
 *   - epochRounds mapping for O(1) lookup by (epochId, epochRoundNumber)
 *   - New views: getLatestEpoch(), getEpochRoundCount(), getEpochRound()
 *   - RoundPosted event includes epochRoundNumber
 *   - expireRound restricted to onlyOracleOrOwner (was public)
 *
 * Error codes:
...
contract CustosMineControllerV053 {
    string public constant VERSION = "v0.5.3";
    ...
    uint256 public roundCount;          // global monotonic counter (never resets)
    uint256 public epochRoundCount;     // per-epoch counter (resets each openEpoch)
    ...
    struct Round {
        uint256 roundId;            // global unique ID
        uint256 epochId;
        uint256 epochRoundNumber;   // 1-based position within epoch (1..140)
        uint256 commitOpenAt;
        uint256 commitCloseAt;
        uint256 revealCloseAt;
        bytes32 answerHash;
        string  questionUri;
        uint256 oracleInscriptionId;
        bool    settled;
        bool    expired;
    ...
    // Epoch-relative round lookup: epochRounds[epochId][epochRoundNumber] = global roundId
    mapping(uint256 => mapping(uint256 => uint256)) public epochRounds;
    // Final round count per epoch (written at finalizeClose)
    mapping(uint256 => uint256) public epochRoundsPosted;
    ...
    // Deduplication for settleRound
    mapping(uint256 => mapping(uint256 => bool))  private _settledInscriptions;
    mapping(uint256 => mapping(address => bool))  private _roundCredited;
    ...
    event RoundPosted(uint256 indexed roundId, uint256 indexed epochId, uint256 epochRoundNumber, string questionUri, uint256 commitOpenAt, uint256 commitCloseAt, uint256 revealCloseAt);
     ...
     * @param questionUri        URI pointing to the oracle's CustosNetworkProxy inscription.
     * @param answerHash         keccak256(abi.encodePacked(correctAnswer)).
     * @param oracleInscriptionId CustosNetworkProxy inscriptionId for the question.
     *
     * E69 now checks the per-epoch round count, not the global roundCount.
     * Global roundCount still increments for unique IDs across epochs.
     */
        ...
        external onlyOracle returns (uint256 roundId)
    {
        ...
        require(epochRoundCount < ROUNDS_PER_EPOCH, "E69");
        ...
        epochRounds[epochId][ern] = roundId;

        emit RoundPosted(roundId, epochId, ern, questionUri, now_, now_ + WINDOW, now_ + WINDOW + WINDOW);
    }
    ...
    function expireRound(uint256 roundId) external onlyOracleOrOwner {
        Round storage r = rounds[roundId];
        require(!r.settled && !r.expired,             "E40");
    ...
    function openEpoch(uint256 startAt) external onlyOracleOrOwner {
        ...
        epochOpen        = true;
        epochRoundCount  = 0;   // reset per-epoch round counter
        snapshotCursor   = 0;
    ...
    function finalizeClose() external onlyOracleOrOwner {
        ...
        Epoch storage ep = epochs[epochId];
        ep.settled       = true;
        ep.claimDeadline = block.timestamp + CLAIM_WINDOW;

        // Persist the epoch's final round count for historical queries
        epochRoundsPosted[epochId] = epochRoundCount;
        ...
        creditCursor     = 0;
        snapshotCursor   = 0;
        emit EpochClosed(epochId, ep.totalCredits, ep.rewardPool);
    }
    ...
    function getCurrentRound() external view returns (Round memory) {
        if (roundCount == 0) return Round({
            roundId: 0, epochId: 0, epochRoundNumber: 0,
            commitOpenAt: 0, commitCloseAt: 0, revealCloseAt: 0,
            answerHash: bytes32(0), questionUri: "",
            oracleInscriptionId: 0, settled: false, expired: false,
            revealedAnswer: "", correctCount: 0
        });
    ...
    /// @notice Returns the current (or most recent) epoch.
    function getLatestEpoch() external view returns (Epoch memory) {
        return epochs[currentEpochId];
    }

    /// @notice Look up a round by its epoch-relative number (1-based).
    function getEpochRound(uint256 epochId, uint256 epochRoundNum) external view returns (Round memory) {
        uint256 rid = epochRounds[epochId][epochRoundNum];
        return rounds[rid];
    }

    /// @notice Number of rounds posted in the given epoch.
    ///         For the current open epoch returns the live count; for closed epochs returns the finalized count.
    function getEpochRoundCount(uint256 epochId) external view returns (uint256) {
        if (epochId == currentEpochId && epochOpen) return epochRoundCount;
        return epochRoundsPosted[epochId];
    }
    ...
    function getTierSnapshot(address w, uint256 e) external view returns (uint256) {
        return tierSnapshot[e][w];
    }

    function getCredits(address w, uint256 e) external view returns (uint256) {
        return epochCredits[e][w];
    }
