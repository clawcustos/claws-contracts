...
/**
 * @title CustosNetworkProxyV057
 * @notice v0.5.7 implementation. Upgradeable proof-of-agent-work network.
 ...
 *
 * v0.5.7 changes vs v0.5.6:
 *   - inscriptionBlockType (slot 38): stores blockType string at inscription time
 *     Enables CustosMineController to verify "mine-commit" type on-chain in settleRound()
 *   - inscriptionRevealTime (slot 39): stores block.timestamp when reveal() is called
 *     Enables MineController to verify reveal happened within round's reveal window
 ...
 */
contract CustosNetworkProxyV057 is
    Initializable,
    UUPSUpgradeable,
    PausableUpgradeable
{
    ...
    // ── Slots 38-40: V0.5.7 mine-commit verification ─────────────────────────
    /// @notice blockType stored at inscription time. e.g. "mine-commit", "build", "research".
    ///         Enables MineController to verify inscription intent on-chain.
    mapping(uint256 => string)  public inscriptionBlockType;         // slot 38

    /// @notice Timestamp when reveal() was called for this inscription.
    ///         Zero until revealed. Enables MineController to verify reveal was in window.
    mapping(uint256 => uint256) public inscriptionRevealTime;        // slot 39

    /// @notice roundId this inscription was submitted for. Zero for non-mine inscriptions.
    ///         roundId is globally unique across epochs — never resets.
    ///         Enables unambiguous round linkage on-chain without epoch context.
    mapping(uint256 => uint256) public inscriptionRoundId;           // slot 40
    ...
    /**
     * @notice V0.5.7 initializer — no state migrations needed.
     *         New slots 38-40 default to zero. Existing inscriptions have no blockType/revealTime/roundId,
     *         which is correct — they predate the mine protocol.
     */
    function initializeV057() external reinitializer(7) {
     ...
     * @param blockType   Category string (e.g. "build", "research", "mine-commit").
     ...
     * @param roundId     V0.5.7: mine round this inscription belongs to. Pass 0 for non-mine inscriptions.
     *                    roundId is globally unique across epochs — never resets on MineController.
     */
    function inscribe(
        bytes32 proofHash,
        bytes32 prevHash,
        ...
        inscriptionBlockType[inscriptionId]   = blockType;      // V0.5.7
        if (roundId != 0) {
            inscriptionRoundId[inscriptionId] = roundId;        // V0.5.7
        }
    ...
    function reveal(
        ...
        inscriptionRevealTime[inscriptionId]      = block.timestamp; // V0.5.7
